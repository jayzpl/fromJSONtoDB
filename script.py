...
db_name = 'persons16.db' #name of ur db
json_file = 'persons.json'
db = SqliteDatabase('%s' % db_name)

class PersonDB(Model):
    gender = CharField()
...
class Data():
    ...
    def check_days(self, dob, dude): #rok przestepny fail
        dob = dob.replace('T',' ')
        dob = dob[0:10]
...
class DataLoad():
    ...
    def most_safe_pass(self,n): # to ogarnij cos zle dziala
        passwords = {}
        for person in PersonDB.select():
            if person.password in passwords:
                ...
                if any(not char.isdigit() and char.islower() and char.isupper() for char in person.password):
